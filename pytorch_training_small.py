...
train_data_dict = train_data_dict[50000:-1000] #this can be optimized to shuffle beforehand for example
train_ds = monai.data.CacheDataset(data=train_data_dict,transform=trainTransforms,cache_rate=1,num_workers=4,progress=True)
...
run = wandb.init(
    ...
    config={
        ...
        'n_train_samples': len(train_data_dict),
        'n_val_samples': len(val_data_dict)
    }
)
